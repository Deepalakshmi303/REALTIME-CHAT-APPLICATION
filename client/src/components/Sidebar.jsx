...
const Sidebar = () => {
    const { getUsers, users, selectedUser, setSelectedUser,
        unseenMessages, setUnseenMessages, deleteChat } = useContext(ChatContext);

    const { logout, onlineUsers, deleteAccount } = useContext(AuthContext)
            ...
            <div className='pb-5'>
                <div className='flex justify-between items-center'>
                    ...
                    bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:block' >
                            ...
                            {selectedUser && (
                                <>
                                    <p onClick={() => deleteChat(selectedUser._id)} className="cursor-pointer text-sm text-red-400">Delete Chat</p>
                                    <hr className="my-2 border-t border-gray-500" />
                                </>
                            )}
                            <p onClick={() => logout()} className="cursor-pointer text-sm">Logout</p>
                            <hr className="my-2 border-t border-gray-500" />
                            <p onClick={deleteAccount} className="cursor-pointer text-sm text-red-500 font-medium">Delete Account</p>
                        </div>
                    </div>
                </div>
